...
const freezePhraseMode = new URLSearchParams(location.search).get('freezephrase') === '1';
if (freezePhraseMode) {
  const hideBtn = document.createElement('button');
  hideBtn.id = 'freezeHidePhrase';
  hideBtn.textContent = 'HIDE REAL PHRASE';
  hideBtn.style.cssText = 'display:block;margin:12px auto;padding:10px 14px;border:1px solid #d9b878;border-radius:8px;background:#171225;color:#f4dfb2;font:12px monospace;z-index:20;position:relative;';
  document.getElementById('hint').insertAdjacentElement('afterend', hideBtn);

  const captureFreeze = (label) => {
    const phrase = document.querySelector('.oracle-phrase');
    const rect = phrase ? phrase.getBoundingClientRect() : null;
    const visibleTextElements = [...document.querySelectorAll('#screen *')]
      .filter((el) => {
        const cs = getComputedStyle(el);
        return Boolean(el.textContent.trim()) && cs.display !== 'none' && cs.visibility !== 'hidden' && Number(cs.opacity) > 0;
      })
      .map((el) => ({ tag: el.tagName, className: String(el.className), text: el.textContent.trim() }));
    const point = rect ? document.elementFromPoint(rect.left + rect.width / 2, rect.top + rect.height / 2) : null;
    return {
      label,
      oraclePhraseCount: document.querySelectorAll('.oracle-phrase').length,
      chCount: document.querySelectorAll('.oracle-phrase .ch').length,
      screenContentHTML: document.getElementById('screenContent').innerHTML,
      visibleTextElements,
      elementFromPoint: point ? { tag: point.tagName, className: String(point.className), text: point.textContent.trim() } : null,
    };
  };

  hideBtn.addEventListener('click', () => {
    const before = captureFreeze('BEFORE_HIDE');
    const phrase = document.querySelector('.oracle-phrase');
    if (phrase) phrase.style.visibility = 'hidden';
    const after = captureFreeze('AFTER_HIDE');
    window.__ALX_FREEZE_REPORT__ = { before, after };
    console.log('[ALX FREEZE REPORT]', window.__ALX_FREEZE_REPORT__);
  });
}
